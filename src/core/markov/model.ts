...
class Model {
    ...
    */
     ...
     * @param rng
     */
    generate(context: string, rng: Rng): string | null {
        const chain = this._chains.get(context);
        if (chain === undefined) {
            return null;
        }
        assert(chain.length > 0);
        return this._alphabet[this.selectIndex(chain, rng)]
    }
    ...
    private selectIndex(chain: number[], rng: Rng): number {
        ...
        const rand = rng.next() * accumulator;
