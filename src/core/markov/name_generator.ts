...
class NameGenerator {
     ...
     */
    generateName(minLength: number, maxLength: number, startsWith: string, endsWith: string, includes: string, excludes: string, regexMatch: RegExp | null = null, rng: Rng): string | null {
        let name: string;

        name = this._generator.generate(rng);
     ...
     */
    generateNames(n: number, minLength: number, maxLength: number, startsWith: string, endsWith: string, includes: string, excludes: string, maxTimePerName = 200, regexMatch: RegExp | null = null, rng: Rng): string[] {
        ...
        while (names.length < n && currentTime < startTime + (maxTimePerName * n)) {
            const name = this.generateName(minLength, maxLength, startsWith, endsWith, includes, excludes, regexMatch, rng);
            if (name != null) {
                names.push(name);
            }
